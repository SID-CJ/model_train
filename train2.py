...
# Define dataset path
dataset_path = "data/train"  # Update this to your dataset location

# Define emotion labels (only angry, sad, happy)
...
for emotion in os.listdir(dataset_path):  # Iterate over emotion folders
    if emotion in emotion_labels:  # Only process specified emotions
...
y = to_categorical(y, num_classes=len(emotion_labels))  # One-hot encoding for 3 classes
...
# 🔹 CNN MODEL (slightly simplified since we have fewer classes)
model = Sequential([
    ...
    Dropout(0.5),
    Dense(len(emotion_labels), activation='softmax')  # Output layer for 3 classes
])
...
# 🔹 SAVE MODEL
model.save("three_emotion_model.h5")
